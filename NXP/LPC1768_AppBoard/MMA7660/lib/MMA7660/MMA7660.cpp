...
void MMA7660::readData(int *data)
{
    ...
    if (!active) {
        setActive(true);
         thread_sleep_for((0.012 + 1/samplerate)/1000); //Wait until new sample is ready, my experience is that 1/samplerate isnt needed, but datasheet says so
    }
...
float MMA7660::getSingle( int number )
{
    ...
    if (!active) {
        setActive(true);
         thread_sleep_for((0.012 + 1/samplerate)/1000); //Wait until new sample is ready
    }
